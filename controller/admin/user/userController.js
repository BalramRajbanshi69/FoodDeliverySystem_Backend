...
exports.getUsers = async(req,res)=>{
    ...
    const users = await User.find({_id: {$ne : userId}}).select(["+otp","+isOtpVerified","-__v"]);          // this means give all those users except the loggedin admin user $ne means not equal
    if(users.length > 1){
        res.status(200).json({
